...
import type { BoardConfig, BoardTileVariable, BoardTileData, LayerData } from '../services/storage/types';
...
export type BoardSubTab = 'Type Board' | 'Tiles' | 'Design';
...
export const Editor: React.FC = () => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === 'z') {
        ...
        if (selectedTileIds.length > 0) {
          ...
          selectedTileIds.forEach(id => {
            if (newData[id]) {
              newData[id] = { id, groupId: newData[id].groupId }; // Keep ID and group
            }
          });
          saveUserAction(newData);
        }
  ...
  const handleSaveBoard = async (config: BoardConfig) => {
    if (name) {
      setAutoSaveStatus('loading');
      setBoardConfig(config);
      const success = await storage.updateDraft(name, { boardConfig: config });
      if (success) {
        setAutoSaveStatus('saved');
        addNotification({
          type: 'success',
  ...
  // Push to history
  const saveUserAction = (newData: Record<number, BoardTileData>) => {
    const newHistory = history.slice(0, historyIndex + 1);
    newHistory.push(newData);
    setHistory(newHistory);
    setHistoryIndex(newHistory.length - 1);
    saveBoardData(boardVariables, newData);
  };
    ...
    <div className="h-screen w-screen flex flex-col bg-meevo-bg text-meevo-text-primary overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 relative flex">
          ...
          {activeTab === 'Board' && boardSubTab === 'Design' && selectedTileIds.length === 1 && selectedLayerIds.length > 0 && (
            <div className="w-80 h-full border-l border-[#CCCCCC]/10 flex flex-col z-10 relative bg-meevo-panel">
              <LayerDetailSidebar 
                selectedTileIds={selectedTileIds}
                boardTilesData={boardTilesData}
